...
### MQTT control loop (command down + state ack up)
The hardware control path mirrors the telemetry contract on two more topics under the same `farm/{org_id}/{device_uid}/...` namespace, where **`device_uid` is the relay-bearing NODE** (e.g. `GH1-NODE-01`) — the actuator (`GH1-VENT-01`) rides in the payload. Config: `settings.mqtt_command_topic_template`, `settings.mqtt_state_topic`, `settings.control_default_driver`. Operator guide: [`hardware-integration.md`](hardware-integration.md).

- **Command DOWN** — `farm/{org_id}/{device_uid}/command`. Published at QoS 1 by `app.control.drivers.mqtt_relay.MqttRelayDriver` (the `mqtt` driver, selected when an actuator's `config.driver == "mqtt"`). Payload: `{command_id, actuator_uid, actuator_type, command, params, ts}`, where `command` ∈ `open|close|on|off`. The driver is **fire-and-forget + fail-soft**: a successful publish returns `CommandResult(ok=True, acked=False)` (the real ack arrives async); a broker/publish failure returns `ok=False` instead of raising, so the service layer degrades to the `mock` driver and the stack still runs offline.
- **State / ack UP** — `farm/{org_id}/{device_uid}/state`. The firmware publishes this **after** the relay physically flips. Payload: `{command_id, actuator_uid, state, ok, ts}` with `state` ∈ `open|closed|on|off`.
- **The `handle_state_message` seam** — `app.control.ingest.handle_state_message(session, *, org_id, device_uid, payload)`. Driven by the MQTT consumer (which also subscribes `settings.mqtt_state_topic`, default `farm/+/+/state`), it resolves the `ActuatorDevice` by `actuator_uid`, confirms `ActuatorDevice.state` + `last_state_change`, marks it online, and flips the correlated `ControlCommand` to `ACKED` (or `FAILED` when `ok` is false). Correlation prefers the explicit `command_id`, else the most recent un-acked command for that actuator. **Idempotent + defensive**: unknown actuators / bad payloads are logged and skipped, never fatal to the consumer. This is what makes the dashboard reflect what the hardware actually did, not just the optimistic state set on publish.
